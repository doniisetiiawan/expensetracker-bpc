...
function ExpenseOverview() {
  ...
  useEffect(() => {
    ...
    currentMonthPreview({ t: jwt.token }, signal).then(
      (data) => {
        if (data.error) {
  ...
  useEffect(() => {
    const abortController = new AbortController();
    const { signal } = abortController;
    expenseByCategory({ t: jwt.token }, signal).then(
      (data) => {
        if (data.error) {
          setRedirectToSignin(true);
        } else {
  ...
  return (
    <Card className={classes.card}>
      ...
      >
        ...
        >
          $
          {expensePreview.month
            ? expensePreview.month.totalSpent
            : '0'}
          {' '}
          <span
            style={{ display: 'block', fontSize: '0.3em' }}
          >
            so far this month
          </span>
        </Typography>
        ...
        {expenseCategories.map((expense, index) => (
          ...
          >
            ...
            <Divider
              ...
              style={{
                backgroundColor: indicateExpense(
